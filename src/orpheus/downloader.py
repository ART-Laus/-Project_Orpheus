...
class Downloader:
    ...
    def _run_albums(self, pending: list[Track], limit: int) -> None:
        ...
        for album_id, tracks in by_album.items():
            ...
            try:
                matched = self.resolver.resolve_album(album, tracks, self.staging)
            except Exception:
                matched = {}
            for track in tracks:
    ...
    def _pending_tracks(self, name_filter: str) -> list[Track]:
        ...
        items.sort(key=lambda t: (not t.get("liked", False), t.get("spotify_id", "")))
        result = []
        for rec in items:
    ...
    def _finalize(self, src: Path, verified: dict, track: Track) -> bool:
        ...
        dest = dest_dir / f"{number:02d}. {title}{src.suffix.lower()}"
        try:
            if dest.exists():
                dest.unlink()
